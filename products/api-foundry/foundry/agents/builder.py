...
class BuilderAgent(StandaloneAgent):
    ...
    def __init__(self, enable_semgrep: bool = False):
    ...
    async def execute(self, context: AgentContext) -> AgentContext:
        ...
        try:
            ...
            # Step 2.5: Generate combined input validation schemas for every endpoint
            import re
            plan.setdefault("schemas", {})
            for tag, endpoints in plan.get("endpoints_by_tag", {}).items():
                for endpoint in endpoints:
                    op_id = endpoint.get("operation_id", "")
                    if not op_id:
                        op_id = f"{endpoint['method'].lower()}_{endpoint['path'].replace('/', '_')}"
                    
                    # Convert to CamelCase name
                    model_name = "".join(x.capitalize() for x in re.split(r'[-_]', op_id)) + "Input"
                    ...
                    for param in endpoint.get("parameters", []):
                        ...
                        props[p_name] = {**p_schema}
                        if p_desc:
                            props[p_name]["description"] = p_desc
                        ...
                        if "content" in req_body:
                            ...
                            if "schema" in json_content:
                                ...
                                if "$ref" in body_schema:
                                    ...
                                    for b_name, b_schema in body_schema["properties"].items():
                                        props[b_name] = b_schema
                                    if "required" in body_schema:
                                        required.extend(body_schema["required"])
                                        
                    # Register this virtual schema
                    plan["schemas"][model_name] = {
                        "type": "object",
                        "properties": props,
                    ...
                    # Link to endpoint
                    endpoint["validation_model"] = model_name
    ...
    def _endpoint_to_tool(self, endpoint: Dict[str, Any], tag: str, schemas: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Track validation model
        validation_model = endpoint.get("validation_model", "Any")
        ...
        if validation_model in schemas:
            input_schema = schemas[validation_model]
